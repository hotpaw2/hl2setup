//
// hl2.c 
// modified for use with a c compiler
// 2020-05-16  rhn
//
...
static int CheckResult(const char * msg, double result, double target, 
			double tolerance, const char * components)
{
	char msg2[120];
	// std::string output;
...
int HL2GetBoardId(void)
{
	...
	if (rx_discover_socket == INVALID_SOCKET) {
		...
		if (rx_discover_socket != INVALID_SOCKET) {
			...
			while(recvfrom(rx_discover_socket, (char *)data, 1500, 0, (struct sockaddr *)&recv_Addr, &addrlen) > 0)
	...
	if (board_id == 0) {
		...
		addrlen = sizeof(recv_Addr);
		i = recvfrom(rx_discover_socket, (char *)data, 1500, 0, (struct sockaddr *)&recv_Addr, &addrlen);
